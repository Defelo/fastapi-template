...
class UserAuth(HTTPAuth):
    def __init__(self, min_level: PermissionLevel):
        super().__init__()
    ...
    async def __call__(self, request: Request) -> Optional[Session]:
        ...
        session: Session = await super().__call__(request)
...
def get_user(*args, require_self_or_admin: bool = False):
    async def default_dependency(user_id: str, session: Optional[Session] = public_auth):
    ...
    async def self_or_admin_dependency(user_id: str = "self", session: Session = user_auth):
